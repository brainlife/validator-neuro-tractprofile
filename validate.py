...
if not os.path.isdir(config["profiles"]):
    ...
    for name in os.listdir(config["profiles"]):
        ...
        if not shape:
            ...
            #    if first != "ad_1" and first != "ad_mean":
            ...
            #    if first != "ndi_1" and first != "ndi_mean":
            ...
            #        results["errors"].append("first column should be ad_1 but it's "+first)
            #else:
            #    results["errors"].append("csv should have 15(noddi only), 19(tensor only) or 31(tensor+noddi) columns. it has "+str(shape[1]));
        else:
            if shape != csv.shape:
                results["errors"].append("csv shape doesn't match for "+name+" "+str(csv.shape))

        results["meta"]["columns"] = csv.columns.tolist()
